...
#define API_DEFAULT_SETUP(X)\
	do {\
		(X)->API.alloc = AMMallocFunc;\
		(X)->API.dealloc = AMFree;\
		(X)->API.cmp = memcmp;\
		(X)->API.copy = memcpy;\
		(X)->API.print = NULL;\
	}while(0)
...
#define func_duplicate_arr_struct(X)\
	F_DUPLICATE(X) {\
			...
			if(!(M(dst) = AMMalloc(obj->capacity * O(obj)))) {\
				AMFree(dst);\
				return NULL;\
			}\
			off = S(obj) * O(obj);\
...
#define create_iter_prototypes(TYPE) \
	CSTL_API iter_proto(prev,TYPE);\
	CSTL_API iter_proto(next,TYPE);\
	CSTL_API iter_proto(assign,TYPE);\
	CSTL_API iter_proto(destroy,TYPE);\
	CSTL_API iter_proto(head,TYPE);\
	CSTL_API iter_proto(tail,TYPE);\
	CSTL_API iter_proto(create,TYPE);\
	CSTL_API iter_proto(retrieve,TYPE);\
	CSTL_API iter_proto(copy,TYPE);\
	CSTL_API iter_proto(swap,TYPE);
...
#define gen_Ptr_Based_iter_create(TYPE) \
		TYPE##Iter* create_##TYPE##Iter(TYPE *obj) {\
			...
			CHECK_VARA((iter = AMMalloc(sizeof *iter)),NULL);\
			iter->ptr = H(obj);\
			iter->parent = obj;\
			return iter;\
		}
...
#define COMMON_FUNCTION_PROTOTYPES(STRUCT)\
	 ...
	 */\
	CSTL_API F_CONSTRUCT_FUNC(STRUCT);\
	/**
	 ...
	 * the object
	 */\
	CSTL_API F_DESTRUCT(STRUCT);\
	/**
	 * @param obj the object to clear
	 * @return 0 on success, non-zero on failure
	 * @brief removes all the data from the object
	 */\
	CSTL_API F_CLEAR(STRUCT);\
	/**
