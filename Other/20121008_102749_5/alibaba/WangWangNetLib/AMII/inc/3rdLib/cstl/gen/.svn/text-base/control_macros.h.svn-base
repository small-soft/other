...
#ifdef AMOS_DEBUG
#define INITIALIZE_NODE_ALLOC(s) alloc((s)->objsize, __FILE__, __LINE__)
#else
#define INITIALIZE_NODE_ALLOC(s) alloc((s)->objsize)
#endif
...
{\
	...
	if((FLAG) == DYNAMIC) {\
		...
		if(!((NEW)->objptr = (STRUCT)->API.INITIALIZE_NODE_ALLOC(STRUCT))) {\
			goto allocobjfail;\
		}\
