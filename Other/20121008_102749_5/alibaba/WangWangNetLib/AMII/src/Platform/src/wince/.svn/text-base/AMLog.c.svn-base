...
static unsigned int AMskip_atou(const char **s)
{
  int i=0;
...
static int AMstrnlen(const char *s, int maxlen)
{
  const char *es = s;
  while ( *es && maxlen ) {
    es++; maxlen--;
...
static int AMdo_div(unsigned long* pNum, unsigned int base)
{
	int __res;
...
static char * number(char * str, long num, int base, int size, int precision, int type)
{
  ...
  if (num == 0)
    ...
    tmp[i++] = digits[AMdo_div((unsigned long*)&num,base)];
...
int AMsprintf(char * buf, const char *fmt, ...);
int AMvsprintf(char *buf, const char *fmt, va_list args)
{
  int len;
  unsigned long num;
  int i, base;
...
int AMprintf(const AMChar* module, const AMChar *fmt, ...)
{
  ...
  AMLogForInfo(module, printf_buf);
