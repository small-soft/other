...
F_CONSTRUCT(OHTable) {
	...
	obj->nodes = AMMalloc(sizeof *obj->nodes * datasize);
...
F_CLEAR(OHTable) {
	...
	for(x = 0; x < obj->capacity; x++) {
		if(obj->nodes[x].objptr) {
			...
			obj->num_elem--;			/*yanlong.shiyl*/
			iter = ptr->next;
			ptr->next = 0;				/*yanlong.shiyl*/
			ptr->objptr = 0;			/*yanlong.shiyl*/
			ptr->objsize = 0;			/*yanlong.shiyl*/
			ptr->flags = 0;				/*yanlong.shiyl*/
			while(iter) {
...
int8_t insert_OHTable(OHTable *table, void *element, size_t elesize,int8_t flag) {
	...
	if(table->nodes[h_index].objptr) {
		...
		} else if(!(ptr = AMMalloc(sizeof *ptr))) {
			return EALLOCF;
		}
		ptr->next = NULL;
		iter->next = ptr;
    ...
    else if((ptr->objptr = table->API.alloc(elesize, __FILE__, __LINE__))) 
    {
#else
	} 
...
F_PRINT(OHTable) {
	...
	return 0;/*yanlong.shiyl !NOT SURE!*/
}
...
F_DUPLICATE(OHTable) {
	...
	OHTableIter *ohiter;
	CHECK_VARN(obj,NULL);
	CHECK_VARN(dst = AMMalloc(sizeof *dst),NULL);
	...
	dst->hash = obj->hash;
	dst->nodes = AMMalloc(sizeof *dst->nodes * dst->capacity);
...
F_CONSTRUCT(CHTable) {
	...
	API_DEFAULT_SETUP(obj);
	if(!(obj->data = AMMalloc(sizeof *obj->data * datasize))) {
		return EALLOCF;
	}
...
#ifdef AMOS_DEBUG
		if((table->data[h_index].objptr = table->API.alloc(elesize, __FILE__, __LINE__))) {
#else
...
F_DUPLICATE(CHTable) {
	...
	dst->hash = obj->hash;
	dst->prob = obj->prob;
	dst->data = AMMalloc(sizeof *dst->data * dst->capacity);
...
int8_t set_hash_CHTable(CHTable *table, int32_t (*hash)(void *,size_t)) {
	CHECK_VARN(table,EINVAL);
	CHECK_VARN(hash,EINVAL);
	table->hash = (unsigned int (*)(void *, unsigned int))hash;
	return SUCCESS;
}
