...
sort_List(List * list, size_t objsize)
{
  ...
  CHECK_VARA((array = AMMalloc(objsize * S(list))), -1);

  FOR_EACH_NODE(iter, list) {
    list->API.copy(&array[x++ * objsize], iter->objptr, objsize);
  }
  qsort(array, S(list), objsize, (int (*)(const void *, const void *))list->API.cmp);
  x = 0;
  FOR_EACH_NODE(iter, list) {
...
rsort_List(List * list, size_t objsize)
{
  ...
  CHECK_VARA((array[0] = AMMalloc(objsize * S(list))), -1);

  x[0] = 0;
  if (!(array[1] = AMMalloc(objsize * S(list)))) {
    ALLOCFAIL(array[1]);
    AMFree(array[0]);
    return -EALLOCF;
  }
...
to_array_List(List * list, size_t objsize)
{
  ...
  CHECK_VARN(list, NULL);
  CHECK_VARE(objsize, NULL);
  CHECK_VARA(array = AMMalloc(objsize * S(list)), NULL);
  x = 0;
  FOR_EACH_NODE(iter, list) {
    list->API.copy(&array[x++ * objsize], iter->objptr, objsize);
  }
