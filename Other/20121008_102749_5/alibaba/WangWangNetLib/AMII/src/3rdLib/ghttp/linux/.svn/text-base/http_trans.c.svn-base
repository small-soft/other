...
//unit of SREAD_TIMEOUT is second
#ifdef ANDROID_BUILD
#define	SREAD_TIMEOUT			2
#elif defined(IOS_BUILD)
#define	SREAD_TIMEOUT			12
#endif
...
http_trans_connect(http_trans_conn *a_conn)
{
  ...
  if (a_conn->hostinfo == NULL)
	...
	#ifdef AMOS_DEBUG
		AMPrintf("http_trans_connect......................%ld\n", a_conn->saddr.sin_addr.s_addr);
	#endif
  ...
  if ((a_conn->sock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
	...
	#ifdef AMOS_DEBUG
		AMPrintf("http_trans_connect.....................socket %d\n", a_conn->sock);
	#endif
	...
	connect(a_conn->sock, (struct sockaddr *)&a_conn->saddr, sizeof(struct sockaddr));
...
static int s_read_timeo(int fd, char *buffer, size_t len, int seconds) {
	int retCode;
	fd_set set;
	struct timeval timeo = {seconds, 0};

	FD_ZERO(&set);
	FD_SET(fd, &set);

	retCode = select(fd+1, &set, NULL, NULL, &timeo);
	
	if (retCode > 0) {
		retCode = read(fd, buffer, len);
	}
	else
		retCode = -1;
...
http_trans_read_into_buf(http_trans_conn *a_conn)
{
  ...
  /* read in some data */  
  //by tom.lih   
  if ((a_conn->last_read = l_read = s_read_timeo(a_conn->sock,
					 &a_conn->io_buf[a_conn->io_buf_alloc],
					 l_bytes_to_read, SREAD_TIMEOUT)) < 0)
    {
      if (errno == EINTR)
	l_read = 0;
      else
