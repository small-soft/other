...
// ====================================== 王险峰添加为了能在 AMSDK 上通用  ====================================================
int AM_minixml_read(int fd, unsigned char	*current, int size)
{
	FILE *pFile = (FILE *)fd;

	return fread(current, size, 1, pFile);
}


int AM_minixml_write(int fd, unsigned char	*current, int size)
{
	FILE *pFile = (FILE *)fd;

	return fwrite(current, size, 1, pFile);
}
...
MINIXML_API mxml_node_t *				/* O - First node or NULL if the file could not be read. */
...
MINIXML_API mxml_node_t *				/* O - First node or NULL if the string has errors. */
...
MINIXML_API char *					/* O - Allocated string or NULL */
mxmlSaveAllocString(
...
MINIXML_API int					/* O - 0 on success, -1 on error. */
...
MINIXML_API int					/* O - Size of string */
...
mxml_fd_read(_mxml_fdbuf_t *buf)		/* I - File descriptor buffer */
{
  ...
  */
  ...
  {
// 王险峰： 因为在AM的实现下不可能进入这个里面，所以为了编译通过，将这块注释掉了。
//#ifdef EINTR
