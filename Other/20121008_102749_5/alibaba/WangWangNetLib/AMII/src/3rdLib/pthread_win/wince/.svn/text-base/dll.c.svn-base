...
)
{
  ...
  switch (fdwReason)
	...
	GetProcAddress(_pthread_h_kernel32,
		       (LPCWSTR) L"TryEnterCriticalSection");

      if (_pthread_try_enter_critical_section != NULL)
        {
          InitializeCriticalSection(&iiics);
          if ((*_pthread_try_enter_critical_section)(&iiics))
            {
              LeaveCriticalSection(&iiics);
            }
          else
            {
              /*
