...
#include <stdlib.h>
#if defined (_WIN32_WCE) && defined(NO_FTIME)
...
UTIL_API int R_RandomInit(random)
...
UTIL_API int R_RandomUpdate(random, block, len)
...
UTIL_API int R_GetRandomBytesNeeded(bytesNeeded, random)
...
UTIL_API int R_GenerateBytes(block, len, random)
...
UTIL_API void R_RandomFinal(random)
R_RANDOM_STRUCT *random;        /* random structure */
{
...
UTIL_API void R_RandomCreate(random)
R_RANDOM_STRUCT *random;                                /* random structure */
{
	clock_t cnow;
	time_t t;
...
UTIL_API void R_RandomMix(random)
