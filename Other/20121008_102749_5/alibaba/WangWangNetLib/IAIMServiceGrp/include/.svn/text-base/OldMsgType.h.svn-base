...
typedef enum _AIMsgType
{
	...
	AUTO_OPERATION_TIP = 5,				//operationTip， utf-8
	...
	GET_DEGREE_FAIL,
	GET_PEER_SEND_VERIFY,				//not reliable. ( on response if contact is offline )
	...
	GET_CNTMEMO_FAIL,
	NTF_PROGRESS = 40,					//文件传输进度 contact是transId, content是进度
	RSP_FILE,							//文件传输结果 contact是transId, content是结果码
	NTF_NEED_AUTH,						//需要验证码 contact是authId, content是base64后的图片
	RSP_AUTH,							//验证码的验证结果 contact是authId, content是验证结果码
