...
extern AMInt32 JavaPreListener(AMVoid *pArg, const AIM_CONTACT_UPDATE_DATA* pPresent, AMInt32 iCnt);
extern AMInt32 JavaMsgListener(AMVoid *pArg, IASession *pSes, IAIMessage *pMsg);
extern AMInt32 JavaConnListener(AMVoid *pvArg, AMInt32 ucType, AMInt32 iInfo);
extern AMInt32 JavaSearchUserListener(AMVoid *pArg, const AIM_RSP_SEARCH_USER *pRspSearchUser);
...
AIM_RESULT	OnNtfDataReady(AMVoid *pArg, AIM_NTF_DATA_READY *ntfDataReady)
{
	...
	case eDATA_BINDID_ALLREADY:
		DPRINT("%s >>>>>> Data Ready\n", __FUNCTION__);
		return (AIM_RESULT)JavaCntListener(pArg, CONTACT_BINDID_LIST_LOADED, 0, 0);
		break;
	case eDATA_BINDID_KICKED:
		DPRINT("%s >>>>>> eDATA_BINDID_KICKED \n", __FUNCTION__);
		return (AIM_RESULT)JavaCntListener(pArg, CONTACT_BINDID_KICKED, 0, 0);
		break;
	}
	return eAIM_RESULT_OK;
}
...
{	
	...
	return (AIM_RESULT)JavaConnListener(pvArg, CONN_CB_GETUSERINFO, pNtfUserInfo->iImlevel<<8 | pNtfUserInfo->iAddcontactFlag);
}
...
AIM_RESULT	OnRspAddContact(AMVoid *pvArg, AIM_RSP_ADD_CONTACT *pRspAddContact)
{
	...
	if(pRspAddContact->szContactId)
	{
		...
		if(eADDCONTACT_SUCCESS == pRspAddContact->eRetCode)
		{
			...
			pMsg->bUnread = AMFALSE;	//remove the short time appearence of '1' in status bar: chunyuan.wuchy
		} 
...
AIM_RESULT	OnRspSearchUser(AMVoid *pvArg, AIM_RSP_SEARCH_USER *pRspGetProfile)
{
	...
	{
		JavaSearchUserListener(pvArg, pRspGetProfile);
	}
	else
	{
		AMAssert(pIM && pIM->pAcnt);
...
//文件传输进度 contact是transId, content是进度
AIM_RESULT	OnNtfProgress(AMVoid *pvArg, AIM_NTF_PROGRESS *pNtfProgress)
{
	IAIMessage stcMsg;
	IAIM *pIM = (IAIM*)(((AMPVoid*)pvArg)[1]);
	AMChar szContact[MAX_INT_NUMBER_LEN] = {0}, szContent[MAX_INT_NUMBER_LEN] = {0};
	
	AMAssert(pvArg && pNtfProgress);

	AMMemset(&stcMsg, 0, sizeof(IAIMessage));
	_AIMItoa(pNtfProgress->iTaskId, szContact);
	_AIMItoa(pNtfProgress->iProgress, szContent);
	stcMsg.szContactID = szContact;
	stcMsg.szContent = szContent;
	stcMsg.eType = NTF_PROGRESS;

	Adapter_OnIncomingSysMsg(pIM->pSesModel, &stcMsg, pvArg);
	
	return eAIM_RESULT_OK;
}

//需要验证码 contact是authId, content是base64后的图片
AIM_RESULT	OnNtfNeedAuth(AMVoid *pvArg, AIM_NTF_NEED_AUTH *pNtfNeedAuth)
{
	IAIMessage stcMsg;
	IAIM *pIM = (IAIM*)(((AMPVoid*)pvArg)[1]);
	AMChar szContact[MAX_INT_NUMBER_LEN] = {0}, *szContent = AMNULL;
	AMInt32 iBase64Len = 0, iRetCode = 0;

	AMAssert(pvArg && pNtfNeedAuth);

	DPRINT("%s >>>>>> Entering, id: %d\n", __FUNCTION__, pNtfNeedAuth?pNtfNeedAuth->iAuthId:0);

	AMMemset(&stcMsg, 0, sizeof(IAIMessage));
	iBase64Len = pNtfNeedAuth->iDataLen * 1.4 + 2;
	szContent = (AMChar *)AMMalloc(iBase64Len);
	if(!szContent)
		return eAIM_RESULT_MALLOC_ERROR;

	AMMemset(szContent, 0, iBase64Len);

	iRetCode = ali_base64_encode(pNtfNeedAuth->pvData, pNtfNeedAuth->iDataLen, szContent, iBase64Len);
	AMAssert(iRetCode>=0);
	
	_AIMItoa(pNtfNeedAuth->iAuthId, szContact);

	stcMsg.szContactID = szContact;
	stcMsg.szContent = szContent;
	stcMsg.eType = NTF_NEED_AUTH;

	Adapter_OnIncomingSysMsg(pIM->pSesModel, &stcMsg, pvArg);

	AMFree(szContent);

	DPRINT("%s >>>>>> Leaving, id: %d\n", __FUNCTION__, pNtfNeedAuth?pNtfNeedAuth->iAuthId:0);

	return eAIM_RESULT_OK;
}

//文件传输结果 contact是transId, content是结果码
AIM_RESULT	OnRspFile(AMVoid *pvArg, AIM_RSP_FILE *pRspFile)
{
	IAIMessage stcMsg;
	IAIM *pIM = (IAIM*)(((AMPVoid*)pvArg)[1]);
	AMChar szContact[MAX_INT_NUMBER_LEN] = {0}, szContent[MAX_INT_NUMBER_LEN] = {0};

	AMAssert(pvArg && pRspFile);

	AMMemset(&stcMsg, 0, sizeof(IAIMessage));
	_AIMItoa(pRspFile->iTaskId, szContact);
	_AIMItoa(pRspFile->eResult, szContent);
	stcMsg.szContactID = szContact;
	stcMsg.szContent = szContent;
	stcMsg.eType = RSP_FILE;
...
//验证码的验证结果 contact是authId, content是验证结果码
AIM_RESULT	OnRspCheckCode(AMVoid *pvArg, AIM_RSP_CHECK_CODE *pRspCheckCode)
{	
	IAIMessage stcMsg;
	IAIM *pIM = (IAIM*)(((AMPVoid*)pvArg)[1]);
	AMChar szContact[MAX_INT_NUMBER_LEN] = {0}, szContent[MAX_INT_NUMBER_LEN] = {0};

	AMAssert(pvArg && pRspCheckCode);

	AMMemset(&stcMsg, 0, sizeof(IAIMessage));
	_AIMItoa(pRspCheckCode->iAuthId, szContact);
	_AIMItoa(pRspCheckCode->iRetCode, szContent);
	stcMsg.szContactID = szContact;
	stcMsg.szContent = szContent;
	stcMsg.eType = RSP_AUTH;
