...
- (UIImage *)arrowImage {
    ...
    CGContextClipToMask(context, rect, [[UIImage imageNamed:@"pull_arrow"] CGImage]);
    CGContextFillRect(context, rect);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    ...
    [outFormat setDateFormat:@"MM月dd日 HH:mm"];
    NSString *timeStr = [outFormat stringFromDate:newLastUpdatedDate];
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新: %@",), newLastUpdatedDate?timeStr:NSLocalizedString(@"",)];
}
...
- (void)triggerRefresh {
    //增加下拉的动画
    CGFloat scrollOffsetThreshold = self.frame.origin.y-self.originalScrollViewContentInset.top;
    [self.scrollView setContentOffset:CGPointMake(0, scrollOffsetThreshold)];
    self.state = SVPullToRefreshStateLoading;
}
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateTriggered:
            titleLabel.text = NSLocalizedString(@"释放可以刷新...",);
            [self rotateArrow:M_PI hide:NO];
            break;
            
        case SVPullToRefreshStateLoading:
            titleLabel.text = NSLocalizedString(@"正在刷新...",);
