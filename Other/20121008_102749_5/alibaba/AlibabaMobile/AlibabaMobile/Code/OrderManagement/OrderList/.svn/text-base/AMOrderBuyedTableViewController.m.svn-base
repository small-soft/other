//
//  HotRankingCompanyTableViewController.m
//  AlibabaMobile
//
...
@implementation AMOrderBuyedTableViewController
...
{
//    NSLog(@"buy tableview frame height = %f",self.m2TableView.frame.size.height);
    NSLog(@"buy tableview frame height = %f",self.m2TableView.frame.size.height);
...
//    isLoading = YES;
    if(!_request){
      self.request = [[AMOrderBuyedRequest alloc] init];  
    }
    ...
    if (loginUserName && loginUserName.length > 0) {
        self.request.buyerMemberId = loginUserName;
...
- (void)loadObjectsFromRemote {
    ...
    [objectManager loadObjectsAtResourcePath:OCEAN_API_URL_ORDER_BUYEDORDERS usingBlock:^(RKObjectLoader* loader) {
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.objects = [[NSMutableArray alloc] init];
        [[NSNotificationCenter  defaultCenter] removeObserver:self  name:NOTIFICATION_ORDER_STATE object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loadRequestWithOrderBuyState:) name:NOTIFICATION_ORDER_STATE object:nil];
        [[NSNotificationCenter  defaultCenter] removeObserver:self  name:NOTIFICAGION_LOGIN_SUCC object:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(buyerArray!=nil&&[buyerArray count]>1){
        ...
        NSDictionary *myDict = [buyedOrders.orderEntries objectAtIndex:0];
        {
            ...
            cell.logoImage1.frame =CGRectMake(20, 135, 60 , 60);
        ...
        if([buyerArray count ]>=2){
            ...
            cell.logoImage2.frame =CGRectMake(90, 135, 60 , 60);
        ...
        if([buyerArray count ]>=3){
            ...
            cell.logoImage3.frame =CGRectMake(160, 135, 60 , 60);
        ...
        if([buyerArray count ]>=4){
            ...
            cell.logoImage4.frame =CGRectMake(230, 135, 60 , 60);
            cell.logoImage4.hidden = NO;
        }
        AMOrderBuyed *buyedOrders = (AMOrderBuyed*)[_objects objectAtIndex:row] ;

        if(!buyedOrders.sellerCompanyName)
        {
            cell.buyerCompanyNameLabel.text = [NSString stringWithFormat:@"无"];
        }else{
            cell.buyerCompanyNameLabel.text = [NSString stringWithFormat:@"%@",buyedOrders.sellerCompanyName];
        }
        cell.buyorsale.text = [NSString stringWithFormat:@"卖家"];
        ...
        BOOL isHaveLogistics = [AMOrderBuyedTableViewController isHasLogisticsInfo:orderStatus];
...
- (IBAction)orderLogisticsAction:(id)sender {
    ...
    viewController.orderId = [NSString stringWithFormat:@"%@",orderModel.identifier];
    viewController.parentTitle = @"买到货品";
    [self.mNavigationController pushViewController:viewController animated:YES]; 
...
-(void)loadRequestWithOrderBuyState:(id)sender{
