...
- (NSDictionary *)fontAttributes
{
	...
	CTFontSymbolicTraits fontSymbolicTraits = [self symbolicTraits];
	
	if (fontSymbolicTraits)
	{
		[traitsDict setObject:[NSNumber numberWithUnsignedInt:fontSymbolicTraits] forKey:(id)kCTFontSymbolicTrait];
	}
...
- (void)setSymbolicTraits:(CTFontSymbolicTraits)fontSymbolicTraits
{
	if (fontSymbolicTraits & kCTFontBoldTrait)
	{
		boldTrait = YES;
	}
	
	if (fontSymbolicTraits & kCTFontItalicTrait)
	{
		italicTrait = YES;
	}
	
	if (fontSymbolicTraits & kCTFontExpandedTrait)
	{
		expandedTrait = YES;
	}
	
	if (fontSymbolicTraits & kCTFontCondensedTrait)
	{
		condensedTrait = YES;
	}
	
	if (fontSymbolicTraits & kCTFontMonoSpaceTrait)
	{
		monospaceTrait = YES;
	}
	
	if (fontSymbolicTraits & kCTFontVerticalTrait)
	{
		verticalTrait = YES;
	}
	
	if (fontSymbolicTraits & kCTFontUIOptimizedTrait)
	{
		UIoptimizedTrait = YES;
	}
	
	// stylistic class is bundled in the traits
	stylisticClass = fontSymbolicTraits & kCTFontClassMaskTrait;   
}
