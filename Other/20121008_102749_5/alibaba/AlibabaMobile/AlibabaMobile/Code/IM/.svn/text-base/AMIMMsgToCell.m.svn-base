...
//    _msgLabel.frame = CGRectMake(self.frame.size.width - 80 - labelWidth, 20, labelWidth, labelHeight);
    [_msgLabel setContentStr:text];
    int labelWidth = _msgLabel.frame.size.width;
    int labelHeight = _msgLabel.frame.size.height;

    _msgLabel.frame =  CGRectMake(self.frame.size.width - 80 - labelWidth, 32, labelWidth, labelHeight);
       
    UIImageView * bkImgView = [[UIImageView alloc] init];
    bkImgView.frame = CGRectMake(self.frame.size.width - 60 - (labelWidth+40), 22, labelWidth+40, labelHeight + 20);
    bkImgView.backgroundColor = [UIColor clearColor];
    
    UIImage * bkImg = [UIImage imageNamed:@"bg_aliww_dailog2"];
