...
{
    if (!_defaultImage) {
        UIImage *image = [UIImage imageNamed:@"top.jpg"];
...
- (void)objectLoader:(RKObjectLoader*)objectLoader didLoadObjects:(NSMutableArray*)objects {
    ...
    AMHomepageDressEvent3Result *result = (AMHomepageDressEvent3Result*)[objects objectAtIndex:0];
    
    if ([result.event3.type isEqualToString:@"event3"]) {
        self.md5Checksum = result.event3.md5Checksum;
        [self.contentData removeAllObjects];
        [self.contentData addObject:result.event3];
        [self resetData];
...
- (void)loadObjectsFromRemote {
	...
	RKObjectMapping* contentMapping = [AMHomepageDressEvent3Result sharedObjectMapping];
    ...
    [objectManager loadObjectsAtResourcePath:OCEAN_API_URL_INDUSTRY_CONTENTS usingBlock:^(RKObjectLoader* loader) {
        ...
        if ([objectManager.acceptMIMEType isEqualToString:RKMIMETypeJSON]) {
            loader.objectMapping = [objectManager.mappingProvider objectMappingForClass:[AMHomepageDressEvent3Result class]];
        }
