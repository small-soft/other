...
@interface AMOperationView ()

@property (nonatomic,retain) HJObjManager *objMan;

@end

@implementation AMOperationView
...
-(AMHomepageDressRequest*)request
{
    if (!_request) {
        ...
        _request.summary = @"";
        _request.configName = @"event1";
        _request.transactionType = AMTRANSACTION_TYPE_HOME_EVENT1_LIST;
    }
    return _request;
}
...
- (void)objectLoader:(RKObjectLoader*)objectLoader didLoadObjects:(NSArray*)objects {
    ...
    if ([objectLoader.params respondsToSelector:@selector(transactionType)] && [objects count] > 0) {
        ...
        else if(transactionType == AMTRANSACTION_TYPE_HOME_PRICE_LIST)
        {
            AMHomepageDressEvent1Result *result = (AMHomepageDressEvent1Result*)[objects objectAtIndex:0];
            
            if ([result.price.type isEqualToString:@"price"]) {
                self.md5Checksum = result.price.md5Checksum;
                [self.contentData removeAllObjects];
                [self.contentData addObject:result.price];
                [self resetData];
